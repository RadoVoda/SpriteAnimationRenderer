...
namespace DOTSSpriteAnimation
{
    public partial class SpriteAnimationRenderer : SystemBase
    {
        ...
        public const float bufferLengthToCapacityThreshold = 0.8f;
        public const int bufferLengthMinimumCapacity = 256;
        ...
        public const string uvBufferName = "uvBuffer";
        public const string indexBufferName = "indexBuffer";
        public const string colorBufferName = "colorBuffer";
        public const string transformBufferName = "transformBuffer";
        public const string paintBufferName = "paintBuffer";
        ...
        {
            foreach (var (indexBuffer, colorBuffer, transformBuffer, data, entity) in
                SystemAPI.Query<DynamicBuffer<SpriteIndexBuffer>,DynamicBuffer<SpriteColorBuffer>, DynamicBuffer<SpriteTransformBuffer>, RefRO<SpriteBufferData>>().WithEntityAccess())
            {
                if (bufferPayloadMap.TryGetValue(entity, out var payload))
                {
                    if (data.ValueRO.length > 0)
                    {
                        payload.SetCount((uint)data.ValueRO.length);
                        payload.SetData(SpriteAnimationRenderer.indexBufferName, indexBuffer.Reinterpret<int>().AsNativeArray(), data.ValueRO.length);
                        payload.SetData(SpriteAnimationRenderer.colorBufferName, colorBuffer.Reinterpret<uint3>().AsNativeArray(), data.ValueRO.length);
                        payload.SetData(SpriteAnimationRenderer.transformBufferName, transformBuffer.Reinterpret<float4x2>().AsNativeArray(), data.ValueRO.length);
            ...
            Dependency = new CheckSpriteLifetimeJob
            {
                storage = storage,
                commands = bufferSystem.CreateCommandBuffer().AsParallelWriter()
            }.ScheduleParallel(Dependency);

            Dependency = new UpdateSpriteEntitiesJob
            {
                storage = storage,
                commands = bufferSystem.CreateCommandBuffer().AsParallelWriter(),
                paintCount = paintCount,
                animBufferMap = animBufferMap,
                sheets = sheets,
                frames = frames
            }.ScheduleParallel(Dependency);

            Dependency = new UpdateSpriteEntityBuffersJob
            {
                sameFrustrum = sameFrustrum,
                cull = cull,
                paintCount = paintCount,
                nextFrustrum = nextFrustrum,
                buffers = buffers,
                indexUpdateMap = indexUpdateMap,
                colorUpdateMap = colorUpdateMap,
                paintUpdateMap = paintUpdateMap,
                transformUpdateMap = transformUpdateMap,
                bufferUpdateMap = bufferUpdateMap
            }.ScheduleParallel(Dependency);
    ...
    public static partial class Extensions
    {
        ...
        {
            a = math.clamp(a, 0, list.Length - 1);
            b = math.clamp(b, 0, list.Length - 1);
            T value = list[a];
            list[a] = list[b];
            list[b] = value;
        }
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static ulong ToMask<T>(this T value) where T : Enum => UnsafeUtility.As<T, ulong>(ref value);

        /// <summary>
        /// Check if any flag bit is set without boxing
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static bool Match<T>(this T mask, T flag) where T : unmanaged, Enum => (mask.ToMask() & flag.ToMask()) != 0;
    }
}